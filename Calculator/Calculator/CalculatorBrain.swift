...
enum CalculatorBrain {
    ...
    var output: String {
        ...
        case .leftOp(left: let left, op: _): result = left
        case .leftOpRight(left: _, op: _, right: let right): result = right
    ...
    private func apply(op: CalculatorButtonItem.Op) -> CalculatorBrain {
        ...
        case .leftOp(left: let left, op: let currentOp):
            ...
            case .plus, .minus, .multiply, .divide:
                return .leftOp(left: left, op: op)
            case .equal:
                if let result = currentOp.calculator(l: left, r: left) {
        ...
        case .leftOpRight(left: let left, op: let currentOp, right: let right):
            switch op {
            case .plus, .minus, .multiply, .divide:
                if let result = currentOp.calculator(l: left, r: right) {
                    return .leftOp(left: result, op: op)
                } else {
                    return .error
                }
